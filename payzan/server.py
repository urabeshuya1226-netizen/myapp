...
class MyHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/':
            kaikei, odai = random_many()
            self.render_template('index.html', kaikei=kaikei, odai=odai)
    ...
    def do_POST(self):
        ...
        if path == '/':
            ...
            kaikei = int(params["kaikei"][0])
            odai = int(params["odai"][0])
            pay_str = params.get('pay', [None])[0]
            pay = nyuuryoku(pay_str)
            if pay is None:
                self.render_template('hantei.html', hantei="数字で入力して下さい。", oturi="-")
                return

            try:
                oturi, hantei = oturi_keisan(kaikei, pay, odai)
            except ValueError as e:
            
                self.render_template('hantei.html', hantei=str(e), oturi="-")
                return
            
            oturi, hantei = oturi_keisan(kaikei, pay, odai)
            self.render_template('hantei.html', oturi=oturi, hantei=hantei)
...
def random_many():
    oturi_kouho = [5, 10, 50, 100, 500]
    odai = random.choice(oturi_kouho)
    kaikei = random.randint(100, 2000)
    return kaikei, odai

def nyuuryoku(pay_str):
    try:
        pay = int(pay_str)
        return pay
...
def oturi_keisan(kaikei, pay, odai):
    oturi = pay - kaikei
    pay_error(oturi)
    if oturi == odai:
        hantei = "成功！"
